...
public sealed class RemoteImage : Image
{
    ...
    protected override void OnPropertyChanged(AvaloniaPropertyChangedEventArgs change)
    {
        ...
        if (change.Property == UrlProperty) { _request?.Cancel(); Source = null; if (this.IsAttachedToVisualTree()) Load(); }
    }
    protected override void OnAttachedToVisualTree(VisualTreeAttachmentEventArgs e) { base.OnAttachedToVisualTree(e); Load(); }
    protected override void OnDetachedFromVisualTree(VisualTreeAttachmentEventArgs e) { _request?.Cancel(); Source = null; base.OnDetachedFromVisualTree(e); }
    private async void Load()
    {
        ...
        try
        {
            ...
            try
            {
                ...
                if (!response.IsSuccessStatusCode || response.Content.Headers.ContentLength > 5_000_000) return;
                ...
                { if (buffer.Length + count > 5_000_000) return; buffer.Write(bytes, 0, count); }
                buffer.Position = 0;
                var bitmap = await Task.Run(() => Bitmap.DecodeToWidth(buffer, 480), request.Token);
